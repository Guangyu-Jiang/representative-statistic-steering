...
gpu="${PPLM_GPU:-0}"
wait_pid="${WAIT_PID:-}"
output="artifacts/pplm_sentiment/corrected_accumulated_output_preservation_validation_top2_w0p05_seeds22_33"
...
CUDA_VISIBLE_DEVICES="${gpu}" PYTHONPATH=src python scripts/run_pplm_sentiment.py \
  ...
  --minimum-norm-steps 5 --minimum-norm-damping 0.5 --ridge 0.01 \
  ...
  --log-probability-preservation-weight 0.05 \
