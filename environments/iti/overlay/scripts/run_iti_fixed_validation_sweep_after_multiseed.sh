...
SETTINGS="validation/iti_generation_fixed_validation_sweep_settings.json"
SOURCE="artifacts/iti_attention_head/generation_validation_sweep_k48"
OUT="artifacts/iti_attention_head/generation_validation_fixed_sweep_k48"
JUDGE="$OUT/local_qwen72b_judge"
LOGS="artifacts/iti_attention_head/logs"
mkdir -p "$OUT" "$JUDGE" "$LOGS"

while pgrep -f '^bash scripts/run_iti_selected_a12_multiseed_after_judges\.sh$' >/dev/null; do
  sleep 30
done
sleep 15
...
run_shard() {
  ...
  env CUDA_VISIBLE_DEVICES="$gpu" \
    ...
    "$PYTHON_BIN" validation/generate_causal_head_perturbation.py \
      ...
      --max-new-tokens 50 --checkpoint-every 2 \
      --reuse-from "$source" \
      --statistics-cache "artifacts/iti_attention_head/statistics/llama2_fold${fold}_k48.npz" \
      --output-dir "$OUT/$tag" \
      >"$LOGS/generation_validation_fixed_sweep_${tag}.log" 2>&1
}
...
env CUDA_VISIBLE_DEVICES=0,1,2,3 \
  ...
  "$PYTHON_BIN" validation/local_truthfulqa_judge.py \
    ...
    --answer-columns fixed_com_k48_a6_answer fixed_com_k48_a8_answer \
      fixed_com_k48_a10_answer \
    >"$LOGS/generation_validation_fixed_sweep_qwen72b_judge.log" 2>&1
...
"$PYTHON_BIN" validation/summarize_local_judge_outputs.py \
  ...
  --judge-model Qwen/Qwen2.5-72B-Instruct \
  --output "$JUDGE/validation_method_comparison.csv"
