...
OUTPUT="artifacts/iti_attention_head/validation_direct_target_k_sweep"
...
for k in "${K_VALUES[@]}"; do
  "$PYTHON_BIN" validation/build_iti_direct_target_settings.py \
    --num-heads "$k" \
    --output "$OUTPUT/settings/k${k}.json"
  for fold in 0 1; do
    source="$STATISTICS/llama2_fold${fold}_k48.npz"
    target="$STATISTICS/llama2_fold${fold}_k${k}.npz"
    if [[ "$k" -lt 48 && ! -s "$target" ]]; then
      "$PYTHON_BIN" validation/slice_iti_statistics.py \
        "$source" --num-heads "$k" --output "$target"
    fi
  done
done
...
run_fold() {
  ...
  env CUDA_VISIBLE_DEVICES="$gpu" \
    ...
    "$PYTHON_BIN" validation/validate_causal_head_perturbation.py \
      ...
      --output-dir "$out" \
      >"$LOGS/direct_target_k${k}_fold${fold}.log" 2>&1
}
...
for k in "${K_VALUES[@]}"; do
  ...
  "$PYTHON_BIN" validation/summarize_causal_head_results.py \
    ...
    >"$LOGS/direct_target_k${k}_summary.log" 2>&1
done
...
"$PYTHON_BIN" validation/summarize_causal_head_results.py \
  ...
  >"$LOGS/direct_target_k_sweep_summary.log" 2>&1
