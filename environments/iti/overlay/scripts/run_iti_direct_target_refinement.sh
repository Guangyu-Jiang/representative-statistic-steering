...
SETTINGS="artifacts/iti_attention_head/validation_direct_target_refinement/settings.json"
OUTPUT="artifacts/iti_attention_head/validation_direct_target_refinement"
LOGS="artifacts/iti_attention_head/logs"
mkdir -p "$OUTPUT/fold0" "$OUTPUT/fold1" "$LOGS"

"$PYTHON_BIN" validation/build_iti_direct_target_refinement.py --output "$SETTINGS"
...
run_fold() {
  ...
  env CUDA_VISIBLE_DEVICES="$gpu" \
    ...
    "$PYTHON_BIN" validation/validate_causal_head_perturbation.py \
      ...
      >"$LOGS/direct_target_refinement_fold${fold}.log" 2>&1
}
...
"$PYTHON_BIN" validation/summarize_causal_head_results.py \
  ...
  >"$LOGS/direct_target_refinement_summary.log" 2>&1
