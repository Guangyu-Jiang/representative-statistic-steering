...
SETTINGS="validation/iti_direct_target_large_cap_settings.json"
OUTPUT="artifacts/iti_attention_head/validation_direct_target_large_caps"
...
run_fold() {
  ...
  env CUDA_VISIBLE_DEVICES="$gpu" \
    ...
    "$PYTHON_BIN" validation/validate_causal_head_perturbation.py \
      ...
      >"$LOGS/direct_target_large_caps_fold${fold}.log" 2>&1
}
...
"$PYTHON_BIN" validation/summarize_causal_head_results.py \
  ...
  >"$LOGS/direct_target_large_caps_summary.log" 2>&1
