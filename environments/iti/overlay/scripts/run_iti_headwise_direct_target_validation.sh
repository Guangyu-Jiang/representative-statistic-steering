...
SETTINGS="validation/iti_headwise_direct_target_settings.json"
OUTPUT="artifacts/iti_attention_head/validation_headwise_direct_target_k48"
...
run_fold() {
  ...
  env CUDA_VISIBLE_DEVICES="$gpu" \
    ...
    "$PYTHON_BIN" validation/validate_causal_head_perturbation.py \
      ...
      >"$LOGS/headwise_direct_target_fold${fold}.log" 2>&1
}
...
"$PYTHON_BIN" validation/summarize_causal_head_results.py \
  ...
  >"$LOGS/headwise_direct_target_summary.log" 2>&1
