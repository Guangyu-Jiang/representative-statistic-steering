...
SETTINGS="validation/iti_group_direction_regularized_heldout_settings.json"
OUTPUT="artifacts/iti_attention_head/heldout_group_direction_regularized_k48"
UNREGULARIZED="artifacts/iti_attention_head/heldout_group_direction_k48/merged"
...
run_fold() {
  ...
  env CUDA_VISIBLE_DEVICES="$gpu" \
    ...
    "$PYTHON_BIN" validation/validate_causal_head_perturbation.py \
      ...
      >"$LOGS/group_direction_regularized_heldout_fold${fold}.log" 2>&1
}
...
"$PYTHON_BIN" validation/summarize_causal_head_results.py \
  ...
  >"$LOGS/group_direction_regularized_heldout_summary.log" 2>&1
...
CANDIDATE="group_direction_probe_min_norm_k48_a4_q1_r0_c2p25"
...
do
  "$PYTHON_BIN" validation/compare_paired_mc.py \
    "$UNREGULARIZED/fold_0_test_results.csv" \
    "$UNREGULARIZED/fold_1_test_results.csv" \
    --candidate-results \
      "$OUTPUT/merged/fold_0_test_results.csv" \
      "$OUTPUT/merged/fold_1_test_results.csv" \
