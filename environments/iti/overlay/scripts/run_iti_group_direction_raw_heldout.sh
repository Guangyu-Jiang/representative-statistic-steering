...
GRID="validation/iti_group_direction_raw_validation_settings.json"
SETTINGS="validation/iti_group_direction_raw_heldout_settings.json"
VALIDATION="artifacts/iti_attention_head/validation_group_direction_raw_k48"
OUTPUT="artifacts/iti_attention_head/heldout_group_direction_raw_k48"
STANDARDIZED="artifacts/iti_attention_head/heldout_group_direction_k48/merged"
...
run_fold() {
  ...
  env CUDA_VISIBLE_DEVICES="$gpu" \
    ...
    "$PYTHON_BIN" validation/validate_causal_head_perturbation.py \
      ...
      >"$LOGS/group_direction_raw_heldout_fold${fold}.log" 2>&1
}
...
"$PYTHON_BIN" validation/summarize_causal_head_results.py \
  ...
  >"$LOGS/group_direction_raw_heldout_summary.log" 2>&1
...
do
  "$PYTHON_BIN" validation/compare_paired_mc.py \
    "$STANDARDIZED/fold_0_test_results.csv" \
    "$STANDARDIZED/fold_1_test_results.csv" \
    --candidate-results \
      "$OUTPUT/merged/fold_0_test_results.csv" \
      "$OUTPUT/merged/fold_1_test_results.csv" \
    --reference "$reference" --settings $CANDIDATES \
    --samples 20000 --seed 42 \
    --output "$OUTPUT/paired_vs_${reference}.csv"
done
