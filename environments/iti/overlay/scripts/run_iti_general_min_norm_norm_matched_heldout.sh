...
SETTINGS="validation/iti_general_min_norm_norm_matched_settings.json"
REFERENCE="artifacts/iti_attention_head/heldout_general_min_norm_expanded_k48"
OUT="artifacts/iti_attention_head/heldout_general_min_norm_norm_matched_k48"
...
run_shard() {
  ...
  env CUDA_VISIBLE_DEVICES="$gpu" \
    ...
    "$PYTHON_BIN" validation/validate_causal_head_perturbation.py \
      ...
      --output-dir "$OUT/$tag" \
      >"$LOGS/general_min_norm_norm_matched_${tag}.log" 2>&1
}
...
"$PYTHON_BIN" validation/summarize_causal_head_results.py \
  ...
  >"$LOGS/general_min_norm_norm_matched_summary.log" 2>&1
...
CANDIDATE="aggregate_com_k48_a20_q0p75_r0_c3"
for reference in fixed_com_k48_a8 fixed_com_k48_a15; do
  "$PYTHON_BIN" validation/compare_paired_mc.py \
    ...
    >"$LOGS/general_min_norm_norm_matched_vs_${reference}.log" 2>&1
done
