...
SETTINGS="validation/iti_group_direction_regularization_stage1_settings.json"
OUTPUT="artifacts/iti_attention_head/validation_group_direction_regularization_stage1_k48"
LOGS="artifacts/iti_attention_head/logs"
mkdir -p "$OUTPUT/fold0" "$OUTPUT/fold1" "$LOGS"

"$PYTHON_BIN" validation/build_iti_group_direction_regularization_stage1.py
...
run_fold() {
  ...
  env CUDA_VISIBLE_DEVICES="$gpu" \
    ...
    "$PYTHON_BIN" validation/validate_causal_head_perturbation.py \
      ...
      >"$LOGS/group_direction_regularization_stage1_fold${fold}.log" 2>&1
}
...
"$PYTHON_BIN" validation/summarize_causal_head_results.py \
  ...
  >"$LOGS/group_direction_regularization_stage1_summary.log" 2>&1
