#!/usr/bin/env bash
set -euo pipefail

gpu="${1:-0}"
root="artifacts/steering_exact_h0_self_classifier_target_pilot"
config="configs/runs/llama_steering_openai_ibm_80_20_clamber_aligned_mean_diff_raw.yaml"
export CUDA_VISIBLE_DEVICES="$gpu"
export PYTHONPATH="${PYTHONPATH:-}:src"

# Use a query-specific clarification-minus-direct hidden direction, but define the
# exact-H0 target from the observed training behavior boundary rather than from
# instruction-token topology.
for quantile in 0.25 0.5; do
  python scripts/run_exact_h0_hybrid_steering.py \
    --config "$config" \
    --datasets ambigqa \
    --artifact-root "$root" \
    --behavior-label-source rule_high_precision \
    --positive-label-mode grounded \
    --topology-controller behavior_lowrank \
    --behavior-rank 1 \
    --direction-source self_contrastive_prompt_pair \
    --direction-readout last_token \
    --retrieval-feature-mode all_layer_plus_exact3 \
    --retrieval-geometry standard \
    --neighbor-ks 20 \
    --target-mode classifier_projection \
    --classifier-target-quantile "$quantile" \
    --topology-alphas 0 1 2 \
    --shared-target-ratios 0.05 0.1 \
    --lambdas 0.1 \
    --dampings 0.01 \
    --trust-ratios 0.1 \
    --gn-steps 12 \
    --optimization-jobs 8 \
    --eval-n 64 \
    --causal-position-beta 2 \
    --topology-decode-mode none \
    --shared-intervention-site layer_output \
    --apply-on prompt_and_decode_shared
done

python scripts/judge_exact_h0_gn_local.py \
  --artifact-root "$root" \
  --batch-size 32
