#!/usr/bin/env bash
# Run the exact-H0 topology steering grid on every eligible held-out test row.
# Usage: scripts/launch_exact_h0_gn_fullheldout_sweep.sh {llama|gemma|mistral} GPU_ID
set -euo pipefail
...
common_args=(
  ...
  --artifact-root artifacts/steering_exact_h0_gauss_newton_fullheldout
  --lambdas 0.1
  --dampings 0.01
...
# These are the two target families and hyperparameter regions from the 100-case development sweep.
