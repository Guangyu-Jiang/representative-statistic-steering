"""Run CLAMBER topology-surrogate steering experiments."""
...
from aen_replication.eval.clamber_topology_steering import run_clamber_topology_steering
...
def main() -> None:
    ...
    setup_logging(
        ...
        Path(config["runtime"]["log_dir"]) / "run_clamber_topology_steering.log",
    )
    set_global_seed(int(config["seed"]))
    append_command_history(config["runtime"]["command_history_path"], sys.argv)
    run_clamber_topology_steering(config=config, seed=int(config["seed"]))
